...
window.addGallery = function (containerId) {
		...
		console.log(data)
		    ...
		    console.log(item);
				...
				console.log('header');
			        ...
			        $(containerId).append('<a href="' + item[2] + '" target="_blank" style="position: relative; width: 320px; height: 450px; margin-bottom: 20px; text-decoration: none;"><div class="publication" style="width: 100%; height: 100%; background-color: rgb(1, 58, 129); background-image: url(\'' + item[3] + '\');background-position: center center; background-repeat: no-repeat; background-size: cover"></div><div style="position: relative; width:100%; padding-top: 15px; padding-bottom: 15px; "><h4 style="margin: 0; padding-left: 15px; padding-right: 15px; text-align: center;">SAS News ' + item[1] + '</h4><div style="position: absolute; bottom: 0px; left: 50%; transform: translateX(-50%); width: 40px; height: 3px; background: rgb(1, 58, 129);"></div></div></a>');
			    }
		    }
                });
            });
